...
BackProjOneView_fan_curve_kernel(float * pfData, float fSinAngle, float fCosAngle, float fFOVRadiusSquare)
{
    ...
    float fX, fY, fColIndex, fU, fDistanceToCenterRay, fOneOverProjDistSquared;
    float * pfVoxel;
    ...
    if(i<_nX_ && j<_nY_)
    {
        ...
        if(fX*fX+fY*fY<=fFOVRadiusSquare)
        {
            ...
            {
                * pfVoxel += tex1D(prjTexture1D, fColIndex) * fOneOverProjDistSquared ;
            }
        }
    }
}
...
backproject_fan_curve(float * image, const float * projection, const float * anglePos)
{
    ...
    // Filtering // 
    projection_ = (float*)malloc(_nProjectionSize*_nSample);
    checkCudaErrors(cudaMemcpy(projection_, projection, _nProjectionSize*_nSample, cudaMemcpyHostToHost));
    float * pfConvKernel = new float[2*_nCols - 1]();
    memset(pfConvKernel, 0, (2*_nCols - 1)*sizeof(float));
    int nIndexRow;
    //  Ramp kernel using the h'' form in Hsieh's book, Eq 3.51
    for(nIndexRow=-_nCols+1;nIndexRow<=_nCols-1;nIndexRow++)	
    {
 	if(nIndexRow == 0)
 	    pfConvKernel[nIndexRow+_nCols-1] = 0.5f/(4* _dCol* _dCol);
	else if(nIndexRow%2)
 	    pfConvKernel[nIndexRow+_nCols-1] = -0.5f/(PI*PI*sinf(nIndexRow*_dCol)*sinf(nIndexRow*_dCol));
 	else
 	    pfConvKernel[nIndexRow+_nCols-1] = 0;
    }	
    ippsMulC_32f_I(_R * _dAngle * _dCol, pfConvKernel, _nCols*2-1);		
    float * pfBuffer = new float[3*_nCols-2];
    // convolution // 
    float * pPrj;
    for(int nAngle=0; nAngle< _nViews; nAngle++)
    {
	for(int nsam=0; nsam< _nSample; nsam++)
	{
	    pPrj = projection_ + nAngle * _nCols * _nSample + nsam * _nCols;
           ippsConv_32f(pPrj, _nCols , pfConvKernel, _nCols*2-1, pfBuffer);	
           memcpy(pPrj, pfBuffer+_nCols-1, _nCols*sizeof(float));
	}
    }
    SAFE_DELETE(pfBuffer);
    SAFE_DELETE(pfConvKernel);
    ////////////////////////////////////////////////////////////////////////////////////// Copy data from host to device ////////////////////////////////////////////////////////////////////////////////////////
    checkCudaErrors(cudaMemcpy(dev_y, projection_, _nProjectionSize*_nSample, cudaMemcpyHostToDevice));
